...
const GameFlexBox = (props) => {
  ...
  const [gameDataView, setGameDataView] = useState(gameData)
  const [sliceArray, setSliceArray] = useState(false)
  ...
  useEffect(() => {
    ...
    if (isFilter) {
      for (let i = 0; i < isFilter.length; i++) {
        filterGameData = filterGameData.filter(item => item.태그[isFilter[i]])
      }
      setGameDataView(filterGameData)
    }
  ...
  return (
    <div className='relative flex flex-wrap text-sm text-center sm:text-base lg:text-lg'>
      ...
      {(sliceArray ? sliceArray[currentPage - 1] : gameDataView).map((item) => (
