...
function Header(props) {
  ...
  const SideBar = () => {
    return (
      <div className={`absolute top-full left-0 ${isOpen ? 'animate-sideBarIn' : 'animate-sideBarOut'}`}>
        <div className={`${sideBarAni ? 'block' : 'hidden'} w-screen sm:w-80 h-screen bg-neutral-900 text-white shadow-xl shadow-sky-500`}>
