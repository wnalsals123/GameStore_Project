...
const ItemDetail = () => {
  ...
  const [gameDetail, setGameDetail] = useState(temp)
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      if (game.length !== 0) setGameDetail(game[0])
      else {
        setTimeout(() => {
  ...
  const GameYoutube = () => {
    if (gameDetail.게임명 === null) {
      ...
      return (
        <iframe className='w-full rounded-lg shadow-lg aspect-video' src={gameDetail.게임명 === null ? "https://cdn-icons-png.flaticon.com/512/66/66165.png" : gameDetail.동영상} title="YouTube video player" frameBorder="0" allow="fullscreen; accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"></iframe>
      )
    }
  }
    ...
    if (gameDetail.게임명 === null) {
      ...
      return (
        <div className='sticky bottom-0 left-0 flex justify-between w-full p-2 text-sm text-center text-white bg-opacity-50 rounded-lg bg-neutral-100 sm:p-5 sm:text-2xl'>
          ...
          <button className='w-[40%] py-1 sm:py-5 bg-sky-500 rounded-xl' onClick={() => { addCart(gameDetail) }}>장바구니</button>
        </div>
      )
    }
  }
